                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if($refund->refund_status == 1)
                                                <span
                                                    class="badge badge-inline badge-success"><strong>{{translate('Approved')}}</strong></span>
                                            @elseif($refund->refund_status == 2)
                                                <span
                                                    class="badge badge-inline badge-danger"><strong>{{translate('Rejected')}}</strong></span>
                                            @else
                                                <span
                                                    class="badge badge-inline badge-warning"><strong>{{translate('PENDING')}}</strong></span>
                                            @endif
                                        </td>
                                        <td>
                                            <a href="{{ route('reason_show', $refund->id) }}"><span
                                                    class="badge badge-inline badge-success">{{translate('Show')}}</span></a>
                                        </td>
                                        <td>
                                            @if($refund->refund_status != 2 && $refund->seller_approval != 2)
                                                @if ($refund->seller_approval == 1)
                                                    <label class="aiz-switch aiz-switch-success mb-0 ">
                                                        <input type="checkbox"
                                                               @if ($refund->seller_approval == 1) checked @endif>
                                                        <span class="slider round"></span>
                                                    </label>
                                                @else
                                                    <label class="aiz-switch aiz-switch-success mb-0">
                                                        <input onchange="update_refund_approval('{{ $refund->id }}')"
                                                               type="checkbox"
                                                               @if ($refund->seller_approval == 1) checked @endif>
                                                        <span class="slider round"></span>
                                                    </label>
                                        ...
                                        <td>
                                            @if($refund->refund_status == 0 && $refund->seller_approval == 0)
                                                <a class="btn btn-soft-danger btn-icon btn-circle btn-sm"
                                                   onclick="reject_refund_request({{$refund->id}})"
                                                   title="{{translate('Reject Refund Request') }}">
                                                    <i class="las la-trash"></i>
                                                </a>
                                            @elseif($refund->seller_approval == 2 || $refund->refund_status == 2)
                                                <a href="javascript:void(0);"
                                                   onclick="refund_reject_reason_show('{{ route('reject_reason_show', $refund->id )}}')"
                                                   class="btn btn-soft-primary btn-icon btn-circle btn-sm"
                                                   title="{{translate('Reject Reason') }}">
                ...
                <form class="form-horizontal member-block" action="{{ route('reject_refund_request')}}" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="form-group row">
                            ...
                            <div class="col-md-9">
                                <textarea type="text" name="reject_reason" rows="5" class="form-control"
