...
class ProfileController extends Controller
{
    ...
    public function updateImage(Request $request)
    {
        ...
        try {
            ...
            if (!empty($tenacy_id)) {
                $dir = public_path('uploads/'.$tenacy_id);
            } else {
            ...
            if (!empty($tenacy_id)) {
                $newPath = "uploads/$tenacy_id/$newFileName";
            } else {
                $newPath = "uploads/all/$newFileName";
            }

            if (env('FILESYSTEM_DRIVER') == 's3') {
                Storage::disk('wasabi')->put($newPath, file_get_contents(base_path('public/') . $newPath));
                Storage::disk('wasabi')->setVisibility($newPath, 'public');
