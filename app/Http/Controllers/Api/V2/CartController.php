...
class CartController extends Controller
{
            ...
            foreach ($owner_ids as $owner_id) {
                ...
                if (!empty($shop_items_raw_data)) {
                    foreach ($shop_items_raw_data as $shop_items_raw_data_item) {
                        ...
                        $product_stock_tmp = $product->stocks->where('variant', $shop_items_raw_data_item['variation'])->first();
                        $shop_items_data_item["upper_limit"] = $product_stock_tmp ? intval($product_stock_tmp->qty)  : 0;
    ...
    {
        Cart::where('user_id', $id)->where('tenacy_id', get_tenacy_id_for_query())->delete();
