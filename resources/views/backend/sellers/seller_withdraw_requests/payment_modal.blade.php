<form class="form-horizontal" action="{{ route('commissions.pay_to_seller') }}"
    ...
    <div class="modal-body">
            ...
            <tbody>
                ...
                <tr>
                    @if ($seller_withdraw_request->amount > $seller->admin_to_pay)
                        ...
                        <td>{{ single_price($seller_withdraw_request->amount) }}
                        </td>
                    @endif
                </tr>
        ...
        @if ($seller->admin_to_pay > 0)
            ...
            <input type="hidden" name="payment_withdraw"
                value="withdraw_request">
            <input type="hidden" name="withdraw_request_id"
                value="{{ $seller_withdraw_request->id }}">
            <div class="form-group row">
                <label class="col-sm-3 col-from-label"
                    for="amount">{{ translate('Amount can pay') }}</label>
                <div class="col-sm-9">
                    @if ($seller_withdraw_request->amount > $seller->admin_to_pay)
                        <input type="number" lang="en" min="0"
                            step="0.01" name="amount" id="amount"
                            value="{{ $seller->admin_to_pay }}"
                            class="form-control" required>
                    @else
                        <input type="number" lang="en" min="0"
                            step="0.01" name="amount" id="amount"
                            value="{{ $seller_withdraw_request->amount }}"
                            class="form-control" required>
                    @endif
                </div>
            </div>
            ...
            <div class="form-group row">
                <label class="col-sm-3 col-from-label"
                    for="payment_option">{{ translate('Payment Method') }}</label>
                <div class="col-sm-9">
                    <select name="payment_option" id="payment_option"
                        class="form-control demo-select2-placeholder" required
                        >
                        <option value="">
                            {{ translate('Select Payment Method') }}</option>
                        @if ($seller->cash_on_delivery_status == 1)
                            <option value="cash">{{ translate('Cash') }}
                            </option>
                        @endif
                        @if ($seller->bank_payment_status == 1)
                            <option value="bank_payment">
                                {{ translate('Bank Payment') }}</option>
                            <option value="momo_payment">
                                {{ translate('Momo Payment') }}</option>
                        @endif
                    </select>
                </div>
            </div>
