<div class="product-item-wrapper">
    <div class="product-item-line">
        <div class="product-item-wrapper-image">
            <img src="{{$productInfo->images[0]}}" alt="image">
        </div>
        <div class="d-flex align-items-center flex-wrap product-item-wrapper-info">
            ...
            <div class="text-ellipsis name">
                ...
                <div class="__react_component_tooltip place-top type-dark"
                     ...
                     style="left: 169px; top: 281px;">{{$productInfo->attributes->name}}</div>
            </div>
            <div class="text-ellipsis variation">({{count($productInfo->skus)}} phiên bản)
            </div>
        </div>
        <div class="d-flex align-items-center text-ellipsis channel-info-wrapper">
            <div class="position-relative d-flex align-items-center text-ellipsis channel-info"
                 role="presentation">
                <img src="{{static_asset('/assets/img/market_place/icon-lazada.png')}}" width="30px">
                &nbsp;&nbsp;
                <span
                    class="text-ellipsis">{{$name[config('market_place.type_market')[3]]['market']}}</span>
            </div>
        </div>
        <div class="d-flex align-items-center text-ellipsis mapped-info-wrapper">
            <div>
                <span class="count-mapping">
                    {{$name[config('market_place.type_market')[3]]['synced']->where('parent_product_market_place_id',$productInfo->item_id )->count()}}
                </span>/<span class="count-variant-product">{{count($productInfo->skus)}}</span>
            </div>
        </div>
        <div class="empty-wrapper"></div>
    </div>
    <div
        class="variant-list-wrapper-{{$productInfo->item_id}}-{{$name[config('market_place.type_market')[3]]['id']}} variant-list-orig">
    </div>
    <div class="show-more">
        <div
            class="more-element-item more-element-item-{{$productInfo->item_id}}-{{$name[config('market_place.type_market')[3]]['id']}}">
            <div class="expand-button">
                <button type="button" class="btn btn-light"
                        onclick="seeMoreChildProduct('{{$productInfo->item_id}}', {{$name[config('market_place.type_market')[3]]['id']}})">
                    <span>Xem thêm</span>
