...
use App\Http\Requests\PosOrderDeliveryRequest;
...
class PosController extends Controller
{
    ...
    public function index()
    {
        if (Auth::user()->user_type == 'admin' || Auth::user()->user_type == 'staff') {
            ...
            return view('pos.index', compact('deliveryPartners'));
        } else {
    ...
    {
        $stocks = Product::where('id', $request->id)->first()->stocks;
        if (count($stocks) > 0) {
    ...
    {
        if (!$request->variant){
            return response_json(false, 'Vui lòng chọn sản phẩm');
        }
    ...
    public function updateQuantity(Request $request)
    {
        ...
        $cart = $cart->map(function ($object, $key) use ($request) {
            if ($key == $request->key) {
                $product = \App\Product::where('id', $object['id'])->first();
        ...
        if (Session::has('posCart') && count(Session::get('posCart')) > 0) {
            ...
            if ($request->user_id == null) {
                ...
                if ($request->shipping_address != null) {
                    $address_data = Address::where('id', $request->shipping_address)->first();
    ...
    public function orderDeliveryPlace(PosOrderDeliveryRequest $request)
    {
        $session = $request->session();

        return $this->repOrder->createOrderDelivery($request->all(), $session);
    }
}
