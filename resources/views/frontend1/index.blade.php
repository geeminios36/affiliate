    ...
    <div class="home-banner-area mb-4 pt-3">
        <div class="container">
            <div class="row gutters-10 position-relative">
                @if (get_setting('home_category_left') == 1 || get_setting('home_category_left') == null)
                ...
                @php
                    $num_todays_deal = count(filter_products(\App\Product::where('published', 1)->where('todays_deal', 1 ))->limit(12)->get());
                    $featured_categories = \App\Category::where('featured', 1)->get();
                @endphp

                <div class="
                    @if (get_setting('home_category_left') == 1 || get_setting('home_category_left') == null)
                        @if($num_todays_deal > 0) col-lg-7
                        @else col-lg-9 @endif
                    @else
                        @if($num_todays_deal > 0) col-lg-10
                            ...
                            @foreach ($slider_images as $key => $value)
                                <div class="carousel-box">
                                    <a href="{{ json_decode(get_setting('home_slider_links'), true)[$key] }}">
                                        <img
                                            class="d-block mw-100 img-fit rounded shadow-sm overflow-hidden {{ (get_setting('home_category_left') == 1 || get_setting('home_category_left') == null) ? '':'h-100' }} "
                ...
                <div class="container">
                    <div class="px-2 py-4 px-md-4 py-md-3 bg-white shadow-sm rounded">
                        <div class="d-flex mb-3 align-items-baseline border-bottom">
                            <h3 class="h5 fw-700 mb-0">
                                <span
                                    class="border-bottom border-primary border-width-2 pb-3 d-inline-block">{{translate('Best Selling') }}</span>
                            </h3>
