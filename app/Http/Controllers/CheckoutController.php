...
use App\Http\Repository\BusinessSettingRepository;
...
class CheckoutController extends Controller
{
    ...
    {
        //
        $this->repBusinessSettings = new BusinessSettingRepository();
    }
        ...
        if ($request->payment_option != null) {
            ...
            if ($request->session()->get('order_id') != null) {
                if ($request->payment_option == 'vnpay') {
                    return $this->repBusinessSettings->getCheckoutVnpay($order, $request->all());
                }

                if ($request->payment_option == 'momo') {
                    return $this->repBusinessSettings->getCheckoutMomo($order);
                }

                if ($request->payment_option == 'appotapay') {
                    $ip = $request->ip();
                    return $this->repBusinessSettings->getCheckoutAppotaPay($order, $request->all(), $ip);
                }
            ...
            foreach ($order->orderDetails as $key => $orderDetail) {
                ...
                if ($orderDetail->product->user->user_type == 'seller') {
                   ...
                   $seller->tenacy_id = get_tenacy_id_for_query(); $seller->save();
                    ...
                    $commission_history->tenacy_id = get_tenacy_id_for_query(); $commission_history->save();
                }
            }
        }
        else{
...
//            if(Auth::user()->club_point->points >= $point) {
    ...
    public function resultMomo(Request $request)
    {
        ...
        if ((int)$inputData['resultCode'] !== 0) {
            flash(translate($inputData['message']))->error();
            return back();
        }

        $payment = ["status" => $inputData['message']];
        $payment_type = \Illuminate\Support\Facades\Session::get('payment_type');
    ...
    public function resultVNPay(Request $request)
    {
    ...
    public function resultAppotaPay()
    {
        $payment = ["status" => 'Thanh toán thành công'];
        $payment_type = \Illuminate\Support\Facades\Session::get('payment_type');
        if ($payment_type == 'cart_payment') {
