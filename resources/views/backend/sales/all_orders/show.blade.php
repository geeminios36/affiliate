...
{{--                @if($delivery_status != 'delivered' && $delivery_status != 'cancelled')--}}
                    <select class="form-control aiz-selectpicker" data-minimum-results-for-search="Infinity"
                        ...
                        <option
                            {{empty($shipping_address) ? '' : (isset($shipping_address->is_hand_over) ? 'disabled' : '')}} value="pending"
                            @if ($delivery_status == 'pending') selected @endif>{{translate('Pending')}}</option>
                        <option
                            {{empty($shipping_address) ? '' : (isset($shipping_address->is_hand_over) ? 'disabled' : '')}} value="confirmed"
                            @if ($delivery_status == 'confirmed') selected @endif>{{translate('Confirmed')}}
                            {{empty($deliveryPartner) ? '' : ('- Tạo đơn '.$deliveryPartner->fullname)}}
                        </option>
                        <option {{empty($deliveryPartner) ? '' : 'disabled'}} value="picked_up"
                                @if ($delivery_status == 'picked_up') selected @endif>
                            {{translate('Picked Up')}} {{empty($deliveryPartner) ? '' : '(Bên đối tác cập nhật)'}}
                        </option>
                        <option {{empty($deliveryPartner) ? '' : 'disabled'}} value="on_the_way"
                    ...
                    <!--<option value="on_delivery" @if ($delivery_status == 'on_delivery') selected @endif>{{translate('On delivery')}}</option>-->
                        <option {{empty($deliveryPartner) ? '' : 'disabled'}} value="delivered"
                                @if ($delivery_status == 'delivered') selected @endif>{{translate('Delivered')}}
                            {{empty($deliveryPartner) ? '' : '(Bên đối tác cập nhật)'}}
                        </option>
                        <option {{empty($deliveryPartner) ? '' : 'disabled'}} {{isset($shipping_address->is_hand_over) ? 'disabled' : ''}}  value="cancelled"
                                @if ($delivery_status == 'cancelled') selected @endif>{{translate('Cancel')}}
                        </option>
                    </select>
{{--                @else--}}
        ...
        <div class="row gutters-5">
            <div class="col text-center text-md-left">
                <address>
                    ...
                    @else
                            <strong class="text-main">{{ $shipping_address->to_name }}</strong><br>
                ...
                <table class="table table-bordered aiz-table invoice-summary">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="width: 25%">
                                @if(!empty($deliveryPartner))
                                    <img src="{{static_asset($deliveryPartner->logo)}}" width="15%">
                                    {{$deliveryPartner->fullname}} <small>{{isset($shipping_address->is_hand_over) ? '('.@$status_delivery.')' : ''}}</small>
                                @endif
            ...
            $.post('{{ route('orders.update_delivery_status') }}', {
                ...
                if (data.code) {
                    if (data.code == 200 || data.success){
                        let message = data.code ? data.message_display : data.message;
